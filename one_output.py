...
n_features = 3
n_pre_min = 1
reframed = series_to_supervised(scaled, n_lookback, n_pre_min)
...
train_X = np.zeros((train.shape[0], 1))
test_X = np.zeros((test.shape[0], 1))
for i in range(n_lookback+1):
	train_X = concatenate((train_X, train[:, 4*i:4*i+3]), axis=1)
	test_X = concatenate((test_X, test[:, 4*i:4*i+3]), axis=1)
train_X = train_X[:, 1:]
test_X = test_X[:, 1:]
...
# design network
model = Sequential()
model.add(GaussianNoise(stddev=0.005, input_shape=(train_X.shape[1], train_X.shape[2])))
...
history = model.fit(train_X, train_y, validation_data=(test_X, test_y),
					epochs=100, verbose=2, shuffle=False, callbacks=[TensorBoard(log_dir='./graph')])
# plot history
...
# # invert scaling for training set
...
# pyplot.subplot(211)
# pyplot.plot(inv_ty, 'r', label='train_a')
# pyplot.plot(y0[:, 0], 'b', label='trained_pre_a')
# pyplot.xlabel('time/10s')
# pyplot.ylabel('acceleration/(m/s²)')
# pyplot.legend()
# pyplot.subplot(212)
# pyplot.plot(inv_y, 'r', label='a')
# pyplot.plot(inv_yhat, 'b', label='pre_a')
# pyplot.xlabel('time/10s')
# pyplot.ylabel('acceleration/(m/s²)')
# pyplot.legend()
# pyplot.show()
